...
async def api_sla_breaches(
    ...
    """Return the number of tickets older than ``sla_days`` that remain open.

    Additional query parameters are treated as equality filters on the
    ``Tickets_Master`` columns. A ``status_id`` parameter may be provided
    multiple times to override the default open status filtering.
    """
    ...
    if status_id is None:
        id_values = request.query_params.getlist("status_id")
        status_id = [int(v) for v in id_values] if id_values else None

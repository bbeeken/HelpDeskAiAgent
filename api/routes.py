...
async def api_get_ticket(ticket_id: int, db: AsyncSession = Depends(get_db)) -> TicketOut:
...
async def api_list_tickets(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[TicketOut]:
    return await list_tickets(db, skip, limit)


@router.get("/tickets/search", response_model=List[TicketOut])
async def api_search_tickets(q: str, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[TicketOut]:
...
async def api_update_ticket(ticket_id: int, updates: dict, db: AsyncSession = Depends(get_db)) -> TicketOut:
...
@router.get("/assets")
async def api_list_assets(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[Any]:
...
@router.get("/vendors")
async def api_list_vendors(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[Any]:
...
@router.get("/sites")
async def api_list_sites(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)) -> List[Any]:
...
async def api_get_ticket_attachments(ticket_id: int, db: AsyncSession = Depends(get_db)) -> List[Any]:
    return await get_ticket_attachments(db, ticket_id)


@router.get("/ticket/{ticket_id}/messages")
async def api_get_ticket_messages(ticket_id: int, db: AsyncSession = Depends(get_db)) -> List[Any]:
    return await get_ticket_messages(db, ticket_id)


@router.post("/ticket/{ticket_id}/messages")
async def api_post_ticket_message(ticket_id: int, msg: MessageIn, db: AsyncSession = Depends(get_db)) -> Any:
    return await post_ticket_message(db, ticket_id, msg.message, msg.sender_code, msg.sender_name)
